...
- В настройках кластера отключили Stackdriver Logging и monitoring;
- Установили Ingress - контроллер Nginx из Helm - чарта;
- Узнали адрес контроллера и добавили запись в hosts;
- Добавили Чарт Прометеуса и загрузили его;
- Подключим сервисы kube-state-metrics и node-exporter;
- Запустили приложение из чарта reddit;
- Добавили отображение label's, присвоенные pod'м;
- Добавили публикацию меток вида __meta_*;
- Отделили target'ы компонент друг от друга;
- Разбили конфигурацию 'reddit-endpoints' на 3 job'а для каждого компонента приложения;
- Установили Grafana из чарта и добавили Прометеус в datasource;
- Добавили дашборд для отслеживания состояния k8s и дашборды для мониторинга компонентов приложения;
- Добавили переменную в дашборд, появился список со значениями переменной;
- Параметризировали созданные ранее дашборды и сохранили в папке kubernetes/dashboards;
- Импортировали смешанный дашборд;
- Логгирование. Добавили label самой мощной ноде в кластере;
- Создали папку с файлами конфигурации и запустили стек;
- Установили Kibana из чарта, запустили веб и подключили fluentd;
- Посмотрели логи компонентов приложения;

ДЗ со *:

- Создали Helm - чарт для стека EFK;
- Файлы поместили в директорию kubernetes/charts/efk, загрузили зависимости (Kibana);
- Удалили текущее окружение EFK и подняли заново с помощью чарта;
...
- Установили Helm, создали и запустили POD Tiller, убедились, что Tiller запущен;
- Создали структуру каталогов Charts, перенесли туда манифесты;
- Установили Chart UI, проверили, что сервис ui доступен;
- Шаблонизируем сущности service, deployment и ingress;
- Установили 3 релиза UI, повявились 3 Ingress - контроллера, UI - интерфейс 
  доступен по всем 3-м адресам контроллера;
- Кастомизируем установку своими переменными (образ и порт), определим их значение
  и обновим инфраструктуру; Убедились, что все релизы приложения доступны;
- Аналогичным образом шаблонизируем сервисы comment и post, добавим helper;
- Определили функции helpers для все сервисов;
- Создадим единый чарт reddit и задали зависимости;
- Добавили Chart mongo в requirements.yaml;
- Подняли окружение reddit, проверили, что UI доступен через Ingress;
- Чтобы UI увидел Post и Comment, добавили в описание переменные окружения;
- После обновления чарта сервис UI увидел Post и Comment;
- Для работы с Gitlab добавили еще один пул в кластер и отключили RBAC;
- Добавили репозиторий gitlab и скачали чарт;
- Поправили конфиги и установили Gitlab;
- Получили доступ к веб - интерфейсу Gitlab, задали root - пароль;
- Задали переменные для доступа к Докер - хабу и создали проект reddit - deploy;
- Создали репозитории приложения и отправили их в Гитлаб;
- Убелились, что пайплайн работает для сервисов ui, post и comment;
- Добавили возможность запускать отдельное окружение и убедились, что оно запускается;
- Добавили stage cleanup, прогнали pipeline и запустили ручное удаление окружения;
- Проверили, что динамическое создание окружений работает для сервисов post и comment;
- Создаем staging и production - среды;
- Убедились, что среда stage поднялась, приложение доступно;
- Вручную запустили развертывание среды deploy и убедились, что приложение доступно в ней;
- 
...
- Отключили сервис kube-dns, убедились, что сервисы недоступны по имени, приложение не работает.
- Включили назад и проверили, что приложение заработало;
- В нашем кластере вместо NodePort перейдем на использования балансировщика;
- Правим конфиги и поднимаем инфраструктуру; убедились, что приложение доступно через балансировщик;
- В веб - интерфейсе GCP посмотрели, как выглядит созданное правило балансировки;
- В консоли GCP включили встроенный Ingress, создали Ingress - правило для сервиса UI;
- Узнали IP - адрес для доступа к Ingress - сервису и убедились, что приложение доступно;
- Отключили балансировщик и настроили Ingress - контроллер для работы через порт 80;
- Настроим TLS - сертификат; генерируем ключи и загружаем в кластер;
- Настроим Ingress - контролеер на прием только HTTPS - трафика;
- Проверили, что приложение открывается через HTTPs;
- Настройка NetworkPolicy для ограничения трафика к базе данных, включение network-policy для GKE;
- Применили политику для mongo, в веб - интерфейсе видим, что сервис post не видит базу данных;
- Изменили настройки манифеста, подключение к БД появилось;
- Создали пост, удалили Mongo - деплоймент, создали заново. База пустая;
- Применили политику для mongo, в веб - интерфейсе видим, что сервис post не видит базу данных;
- Изменили настройки манифеста, подключение к БД появилось;
- Создали пост, удалили Mongo - деплоймент, создали заново. База пустая;
- Создадим отдельное хранилище в облаке для БД и пропишем его в деплоймент - манифест Mongo;
- Пересоздали окружение, снова написали пост и пересоздали окружение; пост остался на месте;
- Создадим PersistentVolume и добавим описание PersistentVolumeClaim;
- Подключили PVC к POD-м и обновили манимест Mongo;
- Создали манифест StorageClass Fast, чтобы монтировать SSD - диски и добавили его в кластер;
- Подключили полученный динамический PVC к POD-м и посмотрели, какие PV получились;

Задание со *:

- Манифест secret для кластера описан в файле http-secret.yml
...
- Подготовим локальное окружение Kubernetes;
- Установили kubectl, установили minikube и запустили кластер. Гипервизор - VirtualBox;
- Убедились, что kubectl види локальный кластер;
- Создадим yaml - файлы для работы приложения Reddit в Kubernetes и каждый компонент запустили
  по отдельности, Пробросим сетевые порты POD-а на локальную машину для теста приложения.
- Сконфигурировали запуска компонента post, пробросили порт 5000 на локальную машину и убедились,
  что приложение работает; также разместили в кластере базу данных;
- Задаем объекты Services для comment,post и mongodb для связи pod - ов друг с другом;
- Подняли инфраструктуру и посмотрели логи pod-ов; Приложение по-прежнему не работает;
- Создали сервис для БД comment и post, подняли все новые сервисы и убедились, что приложение 
  корректно запускается в кластере;
- Создали сервис ui-service для открытия порта доступа к ui - компоненту;
- C помощью minikube service ui убедились, что доступ к приложению снаружи работает;
- Посмотрели список расширений minikube;
- увидели, что расширение dashboard запущено, но его pod мы не видим; переходим к namespaces;
- Посмотрели объекты dash-board в кластере kube-system;
- Познакомились с функционалом веб - интерфейса Dashboard;
- Отделим среду разработки от основного кластера, создадим namespace dev;
- Запустили приложение в dev - namespace, убедились, что оно полноценно работает;
- После работы с локальным кластером переходим к кластеру GKE в облаке;
- Создали кластер с необходимыми параметрами, подключились к нему из консоли через gcloud;
- В результате прописался новый kubeconfig и теперь с помощью kubectl мы можем работать с кластером в облаке;
- Создали dev - окружение (namespace) в кластере и сделали деплой приложения туда;
- В файрволе открыли порты для доступа к приложению снаружи кластера;
- C помощью kubectl узнали внешний IP и порт для доступа к UI;
- Доступ к приложению был успешно получен;
- Запустили dashboard для кластера;
- Запустили прокси дашборда на локальной машине. Заработало только при обращении по ссылке:
  http://localhost:8001/api/v1/namespaces/kube-system/services/https:kubernetes-dashboard:/proxy/#!/login;
- Настроим RBAC для просмотра информации о кластере;
- Теперь дашборд может просматривать информацию о кластере.
